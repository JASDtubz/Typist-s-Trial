//This is the JD Programming syntax version
//I left the variable names alone
...
function checkCorrect(input)
{
    if (mode == "hub")
    {
        if (input == "Warm-up.")
        {
            ...
            {
                if (promptModes[i] == mode) { appendItem(applicablePrompts, prompts[i]); }
            }
        ...
        else if (input == "Trial.")
        {
            ...
            {
                if (promptModes[ii] == mode) { appendItem(applicablePrompts, prompts[ii]); }
            }
    ...
    else if (mode == "warmup")
    {
        if (input == getText("promptLabel"))
        {
            ...
            if (clears == 5)
            {
                ...
                setText(
                    "promptLabel", "Well done, you're ready for trial mode! Type 'Warm-up.' again to try for a better time or 'Trial.' for the real deal. Good luck!"
                );
        ...
        if (input == getText("promptLabel"))
        {
            ...
            if (clears == 5)
            {
                if (time <= bestTrialTime)
                {
                    ...
                    setText("trialBestTimeLabel", "Trial Best Time: " + time + "s");
                }
                ...
                setText(
                    "promptLabel", "Excellent job, you're a keyboard master! Type 'Warm-up.' take a step back or 'Trial.' to try for a better time. Good luck!"
                );
                setText("clearsLabel", "Clears: ___");
                setText("timeLabel", "Time: ___");
            }
...
onEvent("resetKey", "mouseover", function()
{
    ...
    setText(
        "promptLabel",
        "Welcome to the Typist's Trial, this will test your ability to accurately and efficiently replicate displayed text. To begin, please type 'Warm-up.' for a" +
        "simpler challenge or 'Trial.' for the real deal. Good luck!"
    );
    setText(
        "inputLabel",
        "You'll see your typing here, end entries with a punctuation mark to submit. There is no deleting, precision is key. Hover your mouse over the reset" +
        "button to return to this hub at any time."
    );
    setText("clearsLabel", "Clears: ___");
    setText("timeLabel", "Time: ___");
});
